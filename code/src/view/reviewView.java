...
public class reviewView extends JPanel {
    ...
    // Constructor
    public reviewView(Font oswald, Font lato, user user) {
        this.lato = lato;
        ...
        PageTitle.setBackground(new Color (0, 99, 73));
        ...
        // Page details
        details = new JLabel("<html><div style='text-align: center; width: 260px;'>"
                + "<br>Discover insights into the environmental sustainability of"
                + " popular brands. Each brand is rated on a scale from"
                + " 1 to 5 stars, with 5 representing eco-friendly"
                + " practices and 1 indicating poor environmental performance.<br><br>"
                + " Additionally, here are some factors we look at to determine rating:"
                + " materials, transparency, environmental impact, packaging, carbon footprint, ect."
                + "</div></html>");
        ...
        try
        {
            ...
            // Query to get brand names and ratings
            String query = "SELECT brand_name, brand_rating FROM brand";
            ...
            while (rs.next()) {
                brandList.add(new String[]{rs.getString("brand_name"), rs.getString("brand_rating")});
                brandNames.add(rs.getString("brand_name"));
            }
            // Create combo box
            brandBox = createStyledComboBox(brandNames.toArray(new String[0]), lato);
    ...
    private void showBrandReview (String brandName, Font oswald)
    {
        ...
        // Fetch brand rating from brandList
        String brandRating = "0";
        for (String[] brandData : brandList) {
            if (brandData[0].equalsIgnoreCase(brandName)) { // Match brand name
                brandRating = brandData[1]; // Get brand rating
                break;
            }
        }

        System.out.println("Brand: " + brandName + ", Rating: " + brandRating); // Debug

        int rating;
        try {
            // Parse rating as a double
            double rawRating = Double.parseDouble(brandRating.trim()); // Parse the decimal
            rating = (int) Math.round(rawRating); // Round to the nearest whole number
        } catch (NumberFormatException e) {
            System.out.println("Error parsing rating for brand: " + brandName);
            rating = 0; // Default to 0 stars if parsing fails
        }

        // Debug
        System.out.println("Parsed Rating: " + rating);

        if (brandName.equals("--Select Brand--") || brandName.equals("Other"))
        {
            showMainView();
        }
        ...
        if (brandName.equals("Abercrombie & Fitch"))
        {
            ...
            for (int i = 0; i < rating; i++) {
                ...
                mainContentPanel.add(starLabel);
                startX += 60;
            }
        ...
        if (brandName.equals("Free People")) {
            JLabel brand = new JLabel("<html><div style='text-align: center;'>"
                    ...
                    + "supply chain. Additionally, Free People uses materials like leather<br>"
        ...
        if (brandName.equals("Zara")) {
            JLabel brand = new JLabel("<html><div style='text-align: center;'>"
                    + "<br><br><br>The brand has made some efforts toward sustainability, such<br>"
                    + "as setting a science-based target to reduce greenhouse gas emissions<br>"
        ...
        if (brandName.equals("Windsor"))
        {
            JLabel brand = new JLabel("<html><div style='text-align: center;'>"
                    + "REVIEW COMING SOON"
                    + "</div></html>");
            brand.setFont(oswald.deriveFont(28f));
            brand.setHorizontalAlignment(SwingConstants.CENTER); // Center text
            brand.setBounds(-5, 100, 400, 300);
            mainContentPanel.add(brand);

        }

        if (brandName.equals("Stüssy"))
        {
            JLabel brand = new JLabel("<html><div style='text-align: center;'>"
                    + "REVIEW COMING SOON"
                    + "</div></html>");
